...
export async function GET() {
  ...
  const [
    ...
    prisma.searchEvent.count({ where: { clicked: true, clickType: "community" } }),
    prisma.searchEvent.count({ where: { clicked: true, clickType: "external" } }),
  ]);
  ...
  const successRate =
    totalSearches === 0 ? 0 : Number(((successfulSearches / totalSearches) * 100).toFixed(1));
